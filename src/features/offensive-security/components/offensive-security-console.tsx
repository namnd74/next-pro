...
import type { OffensiveSecurityCollection, StudyDossier } from '../types';
...
interface OffensiveSecurityConsoleProps {
  collection: OffensiveSecurityCollection;
}

export function OffensiveSecurityConsole({ collection }: OffensiveSecurityConsoleProps) {
  const { launchedVectorIds, patchedVectorIds, resetOffensiveSecurityProgress } =
    useOffensiveSecurityStore();
  const [mounted, setMounted] = React.useState(false);
      ...
      <Card className="glass-card relative space-y-4 overflow-hidden p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={resetOffensiveSecurityProgress}
