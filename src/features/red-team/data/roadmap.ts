import type { RtPhaseId, RtRoadmapPhase } from '../types';

/**
 * LỘ TRÌNH RED TEAM — chuẩn học thực chiến, thuộc sở hữu của feature RT.
 ...
 */
export const RT_ROADMAP: RtRoadmapPhase[] = [
  {
    id: 'phase-01',
    order: 1,
...
export function getPhaseById(id: RtPhaseId): RtRoadmapPhase | undefined {
  return RT_ROADMAP.find((phase) => phase.id === id);
}

/** Các phase sắp xếp theo thứ tự học */
export const RT_PHASES_ORDERED: RtRoadmapPhase[] = [...RT_ROADMAP].sort(
  (a, b) => a.order - b.order
);
