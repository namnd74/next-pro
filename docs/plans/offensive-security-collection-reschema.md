# Plan: Offensive Security Data Upgrade — Practice Range Collection (Reschema Only)

> ⚡ TRẠNG THÁI: Phase reschema HOÀN TẤT, sau đó mở rộng thành **Offensive Security Academy**
...
> Feature: Offensive Security Academy (`/offensive-security`, `src/features/offensive-security`)
> Decision (user-approved): give the feature its own topic collection decoupled from `/learn`; migrate existing content as-is. No new topics, no content changes.

## 1 · Problem (current coupling audit)

Legacy security data is "made from Learn": every `LegacySecurityScenario.trackSlug` must equal a
`LearningTrack.slug`. Hard dependencies found:

| #   | Coupling                                                                                  | Location                                                |
| --- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| C1  | `scenario.trackSlug` must match `LearningTrack.slug`                                      | `data/json/*.json`, types                               |
| C2  | Track page imports `MOCK_LEARNING_TRACKS` to render the "/learn" link                     | `src/app/offensive-security/[trackSlug]/page.tsx:18,53` |
| C3  | Learn page links `/offensive-security/${track.slug}` assuming slug equality               | `src/app/learn/[trackSlug]/page.tsx:82`                 |
| C4  | `TOPIC_ICON` map duplicated from `/learn` TrackCard                                       | `components/offensive-security-sidebar.tsx:51-62`       |
...
New first-class entity **`OffensiveSecurityCollection`** = one Practice Range topic ("collection")
with its **own slug namespace** and owned presentation metadata:

```ts
// src/features/offensive-security/types/index.ts
export interface OffensiveSecurityCollection {
  id: string;
  slug: string; // feature-owned namespace (no longer required to exist in /learn)
  title: string;
  tagline: string;
...
- `OffensiveSecurityMission.*` (only the file-level key renames)
- Vector ids, mission slugs, all Vietnamese content → **localStorage progress
  (`nextpro-offensive-security-storage`) and all URLs stay valid**
...
- Rename `LegacySecurityScenario` → `OffensiveSecurityCollection`; add `iconName`, `color?`,
...
- `LEGACY_SECURITY_SCENARIOS` → `OFFENSIVE_SECURITY_COLLECTIONS`
- `getLegacySecurityScenarioByTrackSlug()` → `getCollectionBySlug(slug)`
...
- `/offensive-security/[collectionSlug]/page.tsx`: remove `MOCK_LEARNING_TRACKS` import (kills C2);
  render "Học lý thuyết tại /learn" link only when `relatedTrackSlug` is set.
- `/offensive-security/[collectionSlug]/[missionSlug]/page.tsx`: rename-only updates.

### Phase 5 — Components & /learn side

- `offensive-security-sidebar.tsx`: delete `TOPIC_ICON` map (kills C4); use
  `collection.iconName`; rename locals.
- `ui-demo-stage.tsx`: registry keys keep same string values (C5 neutralized —
  keyed by feature-owned slug, works even with no /learn).
- `offensive-security-console.tsx`, `mission-tabs-view.tsx`: prop/type renames only.
...
4. Manual click-through:
   - `/offensive-security` home counts (10 kịch bản · N chiến dịch · vectors)
   - one collection page per difficulty + mobile drawer sidebar
   - one mission page incl. next-mission link
   - `/learn/<track>` ↔ `/offensive-security/<slug>` cross-links both directions
...
User yêu cầu nâng cấp tiếp: Offensive Security phải là **học viện thực chiến độc lập**, không lấy
...
- **Firing range** `components/ranges/*` thay toàn bộ lab React cũ — đăng ký trong
  `UI_DEMO_REGISTRY` theo slug Practice Range.
- **Không còn tham chiếu /learn** nào từ feature Offensive Security (đã xóa relatedTrackSlug,
  cross-link hai chiều, getVectorsByLessonId).

## Trạng thái thực thi

- [x] Types v3 (OffensiveSecurityPhaseId, OffensiveSecurityRoadmapPhase, StudyDossier, OffensiveSecurityCollectionFile)
...
- [x] Sidebar nhóm theo phase; trang chủ /offensive-security render lộ trình
