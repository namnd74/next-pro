# Offensive Security Academy — Left Sidebar + Topic → Lessons → Tabs

> Design proposal (wireframe stage — no code yet).
> Mục tiêu: biến `/offensive-security` thành một "console" có **menu trái theo Topic**, click topic thì
...
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| `/offensive-security`                          | Sidebar trái (10 topics) + khung phải: trang chào/targets overview                                                    | ✅ mới                        |
| `/offensive-security/[trackSlug]`              | Topic đang chọn: sidebar tự expand lesson của topic + khung phải: tổng quan topic (stats, link nhanh tới từng lesson) | ♻️ nâng cấp từ trang hiện tại |
| `/offensive-security/[trackSlug]/[lessonSlug]` | Lesson đang chọn: khung phải chuyển thành **Tabs**                                                                    | ✅ mới                        |
| `/learn/*`, `/interview`                       | Không đổi — chỉ thêm link chéo tới `/offensive-security`                                                              | giữ nguyên                    |

Dữ liệu tái sử dụng 100%:

```txt
Learning tracks  → src/features/learning/data/json/*.json   (lessons: id/slug/title/summary…)
Red team vectors → src/features/offensive-security/data/json/*.json    (3 vectors/track, đã có)
```

---

## 2. Wireframe 1 — `/offensive-security` (mới vào, chưa chọn gì)
...
- `▸` = topic đang đóng. Click → navigate `/offensive-security/[trackSlug]` **và** expand luôn trong sidebar.
- Icon + màu gradient lấy từ `track.color` / `track.iconName` hiện có → đồng bộ với `/learn`.

---

## 3. Wireframe 2 — click 1 topic (ví dụ _Web Security & Auth_)
...
- Lesson item hiện dạng `●` (chưa phá) / `🔴` (đã breach) / `🛡` (đã patch) — đọc từ
  `use-offensive-security-store` (persisted), giống hệt badge trên console.
- Click lesson → `/offensive-security/[trackSlug]/[lessonSlug]`.
...
```txt
src/features/offensive-security/
├─ components/
│  ├─ offensive-security-shell.tsx        (MỚI) layout 2 cột: sidebar + children; client
│  ├─ offensive-security-sidebar.tsx      (MỚI) accordion topics + lesson list + trạng thái store
│  ├─ lesson-tabs-view.tsx      (MỚI) Tabs: Theory | Practice | UI Demo
│  ├─ offensive-security-console.tsx      (giữ) dùng lại trong topic page + nhúng trong Practice tab
│  └─ ui-demo-stage.tsx         (giữ)
├─ data/json-loader.ts          (+ getVectorsByLesson() theo phương án A+C)
└─ stores/use-offensive-security-store.ts (giữ — thêm nothing; sidebar chỉ đọc)

src/app/offensive-security/
├─ layout.tsx                   (MỚI) bọc <OffensiveSecurityShell>
