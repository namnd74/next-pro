...
const contentPath = path.join(
  process.cwd(),
  'src/features/offensive-security/data/academy/roles-and-boundaries.json'
);
const errors = [];
...
try {
  ...
  console.error(`Unable to read academy content: ${error.message}`);
  process.exit(1);
}

requireText(academyModule.id, 'module.id');
requireText(academyModule.slug, 'module.slug');
requireText(academyModule.trackId, 'module.trackId');
requireText(academyModule.title, 'module.title');
requireText(academyModule.summary, 'module.summary');
requireTextArray(academyModule.careerPaths, 'module.careerPaths');
requireTextArray(academyModule.domains, 'module.domains');

if (!Array.isArray(academyModule.lessons) || academyModule.lessons.length !== 3) {
  fail('module.lessons must contain exactly three lessons for this generation batch');
}
...
for (const [lessonIndex, lesson] of (academyModule.lessons ?? []).entries()) {
  const at = `module.lessons[${lessonIndex}]`;
...
if (academyModule.estimatedMinutes !== totalMinutes) {
  fail(
    `module.estimatedMinutes (${academyModule.estimatedMinutes}) must equal lesson total (${totalMinutes})`
  );
}
...
if (errors.length) {
  ...
  console.log(
    `Offensive Security academy content valid: ${academyModule.id}, ${academyModule.lessons.length} lessons, ${totalMinutes} minutes.`
  );
}
